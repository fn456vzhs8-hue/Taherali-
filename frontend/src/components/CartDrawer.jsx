          ...
          <div className="p-5 sm:p-6 border-b border-[#556B2F]/20 flex items-center justify-between">
            <div className="flex items-center gap-3">
              ...
              <div>
                ...
                <p className="text-[11px] text-gray-800 dark:text-[#dcd6bf]">{totalItemsCount} {totalItemsCount === 1 ? "item" : "items"}</p>
              </div>
            </div>
          ...
          <div className="flex-1 overflow-y-auto p-5 sm:p-6 space-y-5">
            {cart.length === 0 ? (
              <div className="text-center py-20 space-y-4">
                <ShoppingBag className="w-16 h-16 text-gray-600 dark:text-[#556B2F] mx-auto" />
                <p className="text-lg font-bold">Your cart is empty</p>
                <p className="text-sm text-gray-800 dark:text-[#dcd6bf]">Add products from Retail or Wholesale to begin your order.</p>
                <button
            ...
            <div className="p-5 sm:p-6 border-t border-[#556B2F]/25 bg-[#F4EEDD]/60 dark:bg-[#243020]/70 space-y-4">
              <div className="space-y-2 text-sm">
                <div className="flex justify-between">
                  <span className="text-gray-800 dark:text-[#dcd6bf]">Total Items</span>
  ...
  return (
    <div className="space-y-3">
      <div className="flex items-center gap-2 text-xs font-bold uppercase tracking-wider text-[#556B2F] dark:text-[#D4AF37]">
        {icon} {title} <span className="text-gray-800 dark:text-[#c9d1c1]">({count})</span>
      </div>
      {items.map(item => (
        <div key={item.id} className="glass-panel p-3.5 rounded-xl border border-[#556B2F]/20 flex items-center gap-3" data-testid={`cart-item-${item.id}`}>
          <div className="flex-1 min-w-0">
            ...
            <p className="text-[11px] text-gray-800 dark:text-[#dcd6bf]">{item.weight} · ₹{item.price} each</p>
