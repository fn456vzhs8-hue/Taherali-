...
import { ArrowLeft, Search, Package, Calculator, ChevronRight } from "lucide-react";
import { PRODUCTS } from "../mock";
import ProductCard from "../components/ProductCard";
import { useShop } from "../context/ShopContext";

export default function ProductsPage() {
  const { setBulkCalculatorOpen } = useShop();
  ...
  return (
    <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8 animate-fadeIn" data-testid="products-page">
      ...
      <nav className="flex items-center gap-2 text-xs sm:text-sm text-gray-500 dark:text-[#c9d1c1] mb-6" data-testid="products-breadcrumb">
        ...
        <span className="text-[#2C3E1F] dark:text-[#F5F1E4] font-semibold">Products Catalog</span>
      </nav>
      ...
      <div className="flex flex-col sm:flex-row sm:items-end sm:justify-between gap-4 mb-8">
        ...
        <button
          ...
          className="inline-flex items-center gap-2 px-5 py-3 rounded-xl bg-[#D4AF37] hover:bg-[#c59d2e] text-[#2C3E1F] font-bold shadow-md transition-all w-fit"
        >
      ...
      {loading ? (
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-8" data-testid="product-skeleton-grid">
          {Array.from({ length: 8 }).map((_, i) => (
            <div key={i} className="glass-panel p-5 rounded-2xl border border-[#556B2F]/20 skeleton-card">
